...
const paths = {
  freeze: new URL("../artifacts/compiled-semantic-lexicon-v1/CSL-03_PRE_OPEN_FREEZE.json", import.meta.url),
  scorerFreeze: new URL("../artifacts/compiled-semantic-lexicon-v1/CSL-03_SCORER_FREEZE.json", import.meta.url),
  ...
  fixture: new URL("../fixtures/compiled_semantic_v1/blind_v1.json", import.meta.url),
  index: new URL("../artifacts/compiled-semantic-lexicon-v1/CSL-01_INDEX.json", import.meta.url),
  runtime: new URL("../runtime/compiled_semantic_v1/router.mjs", import.meta.url),
  result: new URL("../artifacts/compiled-semantic-lexicon-v1/CSL-03_TEST_V1_RESULT.json", import.meta.url),
};
...
function verifyFrozenInputs() {
  ...
  for (const [key, field] of [
    ["fixture", "test_v1_fixture_sha256"],
  ...
  if (freeze.test_v1_evaluation_invocations !== 0) throw new Error("TEST v1 already opened");
}
...
if (process.argv.includes("--verify-result")) {
  ...
  if (result.pre_open_freeze_sha256 !== sha(bytes("freeze")) ||
      result.fixture_sha256 !== freeze.test_v1_fixture_sha256 ||
  ...
  if (fs.existsSync(paths.result)) throw new Error("TEST v1 was already consumed; refusing rerun");
  const { createCompiledRouter } = await import("../runtime/compiled_semantic_v1/router.mjs");
  const router = createCompiledRouter(JSON.parse(bytes("index").toString("utf8")), freeze.selected_policy);
  ...
  if (fixture.counts.single_label !== 288 || fixture.counts.context !== 72 ||
      ...
      fixture.cases.length !== 396) throw new Error("Frozen TEST v1 counts changed");
  ...
  const artifact = { format: "csl03-blind-result-v1", verdict: pass ? "PASS" : "FAIL",
    pre_open_freeze_sha256: sha(bytes("freeze")), fixture_sha256: sha(bytes("fixture")),
    candidate_main_sha: freeze.candidate_main_sha, evaluation_invocations: 1,
